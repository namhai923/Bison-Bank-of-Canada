...
import  { vSchema } from "../views/authentication/auth-forms/AuthLogin";
...
describe('Login', () => {
    ...
    test('See if validation can detect valid emails', async () => {
        ...
        expect(await vSchema.isValid({email: 'a@b.c'})).toBe(true);
        expect(await vSchema.isValid({email: 'Jessewu1999@gmail.com'})).toBe(true);
        expect(await vSchema.isValid({email: 'daddybezos@amazon.com'})).toBe(true);
    })
