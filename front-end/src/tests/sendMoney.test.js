...
import Transfer from "../views/banking/send-money/Transfer"
import { Provider } from 'react-redux';
import store from "../store"
import { BrowserRouter } from 'react-router-dom';
import config from '../config';
import '@testing-library/jest-dom';
import { vSchema  } from '../views/banking/send-money/Transfer';
...
describe('Transfer Money', () => {
    ...
    test('See if validation can detect valid emails and amounts', async () => {
        ...
        expect(await vSchema.isValid({receiver: 'daddybezos@amazon.com', amount: 100.10})).toBe(true);
    })
    test('See if validation can detect invalid emails', async () => {
        ...
        expect(await vSchema.isValid({receiver: 'daddydaddydaddydaddydaddydaddydaddydaddydaddydaddydaddy@daddy.com', amount: 1})).toBe(false);
    })
    test('See if validation can detect invalid amounts', async () => {
        ...
        expect(await vSchema.isValid({receiver: 'daddybezos@amazon.com', amount: -100.10})).toBe(false);
    })
})
