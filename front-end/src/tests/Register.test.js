...
import Register from "../views/authentication/auth-template/Register"
...
describe('Register', () => {
    ...
    test('See if the validation schema can detect that a valid account is being created', async () => {
        ...
        expect(await vSchema.isValid({email: 'a@b.c', fname:'Elon', lname: 'musk', balance: 100})).toBe(true);
        expect(await vSchema.isValid({email: 'daddybezodiz@amazon.com', fname:'Daddy', lname: 'Bezos', balance: 0})).toBe(true);
        expect(await vSchema.isValid({email: 'elonmusk@twitter.com', fname:'Elon', lname: 'Musk', balance: 100})).toBe(true);
    })
    ...
    test('See if the validation schema can detect an invalid email', async () => {
        ...
        expect(await vSchema.isValid({email: 'daddydaddydaddydaddydaddydaddydaddydaddydaddydaddydaddy@daddy.com', fname:'Elon', lname: 'Musk', balance: 100})).toBe(false);
       
    })

    test('See if the validation schema can detect an invalid first name', async () => {
        expect(vSchema.__isYupSchema__);
        expect(await vSchema.isValid({email: 'elonmusk@twitter.com', fname:'daddydaddydaddydaddydaddydaddydaddydaddydaddydaddydaddy', lname: 'Musk', balance: 100})).toBe(false);
    ...
    test('See if the validation schema can detect an invalid last name', async () => {
        expect(vSchema.__isYupSchema__);
        expect(await vSchema.isValid({email: 'elonmusk@twitter.com', fname:'Elon', lname: 'daddydaddydaddydaddydaddydaddydaddydaddydaddydaddydaddy', balance: 100})).toBe(false);
    ...
    test('See if the validation schema can detect an invalid balance number', async () => {
        ...
        expect(await vSchema.isValid({email: 'elonmusk@twitter.com', fname:'Elon', lname: 'Musk',balance: 999999999999999999})).toBe(false);
